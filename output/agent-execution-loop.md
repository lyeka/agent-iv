...
本文分为两部分。第一部分沿着“最小循环—源码实现—生产环境中的异常处理”的顺序，阅读 PI 与 Claude Code 的源码；第二部分把主要结论整理成一套适合面试口述的分析式回答。
...
到这里，第一部分已经从最小循环走完模型响应、工具执行、停止判断、截断、fallback、流式处理、结构化输出和请求重试。下面不再增加新的实现抽象，而是把这些源码结论转换成面试中的口头回答。
...
一次 turn 通常包含：准备上下文和工具定义、调用模型并逐段接收流式响应、确认最终响应、提取完整工具调用、校验参数与权限、串行或并行执行工具、按原顺序记录结果、累计 usage，最后判断继续或终止。状态机至少要区分当前 turn 和 model attempt、模型响应是否已经最终确认、工具调用处于等待、执行、完成还是取消状态，以及累计用量和最终结束原因，不能只从消息数组临时反推这些事实。
